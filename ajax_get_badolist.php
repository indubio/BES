 ...
 * Filtering
 */ 
$sWheretotal1 = "WHERE (geschlossen=0 OR (geschlossen=1 AND entlassungsdatum='')) AND (datamigration!=1) AND (`cancelled`=0)";
...
if (isset($_GET['selstation']) and ($_GET['selstation'] != "undefined")){
	...
	$selectedstation = $_SESSION['stationsid'];
	if ($_SESSION['userlevel'] == UG_STATUSER or $_SESSION['userlevel'] == UG_ADMIN or $selectedstation == -1) {
		$selectedstation=99;
	}
}
 ...
 * Get data to display
 */
$sQuery  = "(SELECT '1' as `u_db_tbl`, `ID` as `u_ID`, `familienname` as `u_familienname`, `vorname` as `u_vorname`, `behandler` as `u_behandler`, `station_c` as `u_station`, `geburtsdatum` as `u_geburtsdatum`, `geschlossen` as `u_geschlossen`, `datamigration` as `u_datamigration`, `reopen` as `u_reopen`, '0' as `u_badotyp` ,`aufnahmedatum` as `u_aufnahmedatum`, `entlassungsdatum` as `u_entlassungsdatum`, `aufnahmenummer` as `u_aufnahmenummer`, `last_change` as `u_last_change`, `entlassungsdatum` as `u_entlassdatum` from fall $sWhere1) UNION ";
$sQuery .= "(SELECT '2' as `u_db_tbl`, `ID` as `u_ID`, `familienname` as `u_familienname`, `vorname` as `u_vorname`, `behandler` as `u_behandler`, `pia_id`    as `u_station`, `geburtsdatum` as `u_geburtsdatum`, `geschlossen` as `u_geschlossen`, '0' as `u_datamigration`, '0' as `u_reopen`, `badotyp` as `u_badotyp`,`aufnahmedatum` as `u_aufnahmedatum`, `entlassdatum` as `u_entlassungsdatum`, `soarian_aufnahmenummer` as `u_aufnahmenummer`, `last_change` as `u_last_change`, `entlassdatum` as `u_entlassdatum` from fall_pia $sWhere2) $sOrder $sLimit";
...
$sQuery = "SELECT sum(num_ids) FROM ((SELECT COUNT('ID') as num_ids FROM fall $sWheretotal1) UNION (SELECT COUNT('ID') as num_ids FROM fall_pia $sWheretotal2)) AS countsubset";
...
while ( $aRow = mysql_fetch_array( $rResult ) ){
	...
	if ($aRow['u_reopen'] == 1) {
		...
		if ($aRow['u_last_change'] == "0000-00-00 00:00:00") { $statusdummy = "A"; }
